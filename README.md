...
A full-stack B2B prospecting pipeline that discovers startups, enriches them with firmographic data, scores them against your ICP, deep-researches founders for personalisation, and manages outreach through a Dream 100 sequence. Built for a growth consultancy's daily workflow, now open-sourced as a reference implementation of what's possible when you wire [Claude Code](https://docs.anthropic.com/en/docs/claude-code) skills into a real product.
...
Prospectatron automates the boring parts (discovery, firmographics, scoring) and the time-consuming parts (founder deep-dives, CRO opportunity spotting, outreach angle generation) while keeping the human in the loop for judgment calls. The operator decides who to pursue and what to say. The system does the legwork.

This was built for my own daily use with no initial intention to release it. It's tuned to a specific ICP (B2B SaaS startups that need growth help), a specific outreach method (personalised Loom recordings), and a specific database of sources (YC, ProductHunt, SBIR grants, accelerator portfolios). Adapting it to your ICP and outreach style is straightforward — the pipeline structure is universal, only the scoring rubric and skill instructions change.
...
│  (18 scripts)       │         │  pipeline_runs│         │  /dream-100/pipeline │
│                     │         │               │         │  /research/{domain}  │
└─────────────────────┘         └───────────────┘         └──────────────────────┘
```
...
```
/prospect seed  →  /prospect daily  →  Dashboard /triage  →  /research  →  Dream 100
     │                   │                    │                   │              │
  Gather from         5 phases:          Card-stack UI:      5-phase web    8-step, 30-day
  YC, PH, SBIR,     1. Pick 20         select / skip /     research per    outreach sequence
  accelerators       2. Pre-filter      snooze / dismiss    company with    with Kanban board
  into pool          3. Enrich                              founder deep-   and touchpoint
                     4. Score                               dive, CRO       logging
                     5. Report                              analysis
```

### Phase 1: Seed — Build the Pool

Run all gathering scripts to populate Postgres with companies from multiple sources:

| Script | Source | What it finds |
|--------|--------|---------------|
| `gather_yc.py` | Y Combinator API | Recent batches, filtered by team size |
| `gather_producthunt.py` | ProductHunt | Trending launches |
| `gather_accelerators.py` | Firecrawl | EF, Seedcamp, Alchemist, Techstars, Antler portfolios |
| `gather_sbir.py` | SBIR.gov API | NSF Phase I software awards |
| `gather_hn.py` | Hacker News Algolia | Show HN posts with 5+ points |
| `gather_github.py` | GitHub API | Recently created orgs |
| `gather_directories.py` | YAML-configured | Any accelerator portfolio page |

Each script upserts directly into Postgres. Partial results are fine — if one script fails, the others still populate.

### Phase 2: Daily Pipeline — Pick, Filter, Enrich, Score, Report
...
2. **Pre-filter (LLM)** — Claude Code applies the "Loom Test": could you record a 2-minute walkthrough of their product showing growth opportunities? Rejects marketing platforms, deep-tech, non-software, non-startups. Fixes missing URLs via web search (never filters by URL status). Max 15 companies proceed.
...
4. **Score** — Two-pass LLM scoring. Pass 1 extracts observable facts. Pass 2 scores 1-5 on 7 weighted criteria (team size, funding, growth motion, traction, growth hire absence, founder identified, timing signals). Score 0-100 with time decay (100% at 0-30 days, 75% at 31-60, 50% at 61-90, 0% after 90).

5. **Report** — Generates markdown, JSON, and CSV shortlists.

### Phase 3: Triage — Morning Briefing
...
### Phase 4: Research — Deep Personalisation
...
# Optional — enables GitHub source gathering
...
make seed     # Populates the pool from all sources (optional, takes a few minutes)
...
Then open Claude Code in the project directory and run `/prospect seed` to build your pool, or `/prospect daily uk` to run a daily pipeline.


## Adapting for Your ICP

The pipeline is generic. The scoring is specific. To adapt Prospectatron for your use case:

1. **Edit `references/icp-criteria.md`** — Define your ideal customer profile, hard disqualifiers, and target signals.
2. **Edit `references/scoring-rubric.md`** — Adjust the 7 scoring criteria, weights, and threshold definitions.
3. **Edit the pre-filter rules in `skills/prospect/SKILL.md`** — The "Loom Test" and rejection criteria are in Phase 2. Replace with your own qualification logic.
4. **Edit `prompts/research-methodology.md`** — The research phases and output schema. Adjust for your outreach style.
5. **Edit `references/sources.md`** — Add or remove gathering sources.

The pipeline structure, database schema, dashboard, and enrichment scripts don't change. Only the judgment criteria.
...
| Enrichment | Python 3 | 18 scripts across 4 tiers. Parallel execution with per-tier timeouts. Fail-open design. |
...
│   │   └── references/      # ICP criteria, scoring rubric, sources, tool signatures
│   └── research/
...
│   ├── gather_*.py          #   7 source gatherers
│   ├── pipeline.py          #   Pipeline entrypoint
│   ├── pool_db.py           #   Shared Postgres utilities
...
├── references/              # ICP criteria, scoring rubric, sources
...
make seed              # Run all gathering scripts
